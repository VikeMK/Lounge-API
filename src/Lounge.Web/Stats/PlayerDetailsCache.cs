...
namespace Lounge.Web.Stats
{
    public class PlayerDetailsCache : IPlayerDetailsCache, IDbCacheUpdateSubscriber
    {
        ...
        private IReadOnlyDictionary<(GameMode Game, int Season), IReadOnlyDictionary<int, PlayerDetails>> _playerDetailsLookupBySeason = new Dictionary<(GameMode Game, int Season), IReadOnlyDictionary<int, PlayerDetails>>();
        ...
        public bool TryGetPlayerDetailsById(int playerId, GameMode game, int season, [NotNullWhen(returnValue: true)] out PlayerDetails? playerDetails)
        {
        ...
        {
            var newPlayerDetailsLookup = new Dictionary<(GameMode Game, int Season), IReadOnlyDictionary<int, PlayerDetails>>();
            ...
            foreach (var game in _loungeSettingsService.ValidGames)
            {
                ...
                foreach (var season in seasons)
                {
                    var placementsByPlayer = dbCache.Placements.Values
                        .Where(p => p.Season == season && p.Game == game)
                        .GroupBy(p => p.PlayerId)
                        .ToDictionary(p => p.Key, p => p.Select(x => x.Id).ToList());

                    var tableScoresByPlayer = dbCache.TableScores.Values
                        .SelectMany(s => s.Values)
                        .Where(p => dbCache.Tables.TryGetValue(p.TableId, out var table) && table.Season == season && table.Game == game)
                        .GroupBy(p => p.PlayerId)
